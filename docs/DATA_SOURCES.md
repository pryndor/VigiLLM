...
**Tested constraint (2026-09-15)**: openFDA's aggregation (`count`) query used to pull reaction terms caps at 1000 results per request with no pagination support (`skip` param is rejected outright when combined with `count`). This means the openFDA-bootstrap approach tops out around the 1000 most-frequent reaction terms — real coverage, but nowhere near MedDRA's full ~24,000 PT list. Fine for an early retrieval-index prototype (tested working on a 300-term sample, retrieval quality good); before Phase 4 production use, either accept the top-1000-frequency coverage gap explicitly, or resolve the MedDRA licensing question below to get the full term list.
